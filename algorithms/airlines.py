...
def calculate_flight_budget(graph: Graph, origin: int, stopover_budget: int, monetary_budget: int) -> ExtensibleList:
    ...
    distances = ExtensibleList()
    stopovers = ExtensibleList()
    for _ in graph._nodes:
        distances.append(float('inf'))
        stopovers.append(-1)
    distances.set_at(origin, 0)
    stopovers.set_at(origin, 0)
    ...
    while not queue.is_empty():
        ...
        current_stopover = stopovers.get_at(current_node)

        # If we've already processed a better path to this node, skip
        if current_cost > distances.get_at(current_node):
            continue
        ...
        for neighbour, edge_cost in graph.get_neighbours(current_node):
            ...
            if current_stopover + 1 <= stopover_budget and current_cost + edge_cost < distances.get_at(neighbour_id):
                distances.set_at(neighbour_id, current_cost + edge_cost)
                stopovers.set_at(neighbour_id, current_stopover + 1)
                queue.insert(distances.get_at(neighbour_id), neighbour_id)

    # Filter destinations based on monetary budget and sort them
    destinations = ExtensibleList()
    i = 0
    while i < distances.get_size():
        if distances.get_at(i) <= monetary_budget:
            destinations.append((i, distances.get_at(i), stopovers.get_at(i)))
        i += 1

    # Sort destinations using the provided sort method in ExtensibleList
    destinations.sort()
